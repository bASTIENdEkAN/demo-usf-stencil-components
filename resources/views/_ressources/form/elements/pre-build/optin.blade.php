<div class="control">
    <label class="checkbox @if(!empty($isFloat)) is-float @endif" for="{{$name}}">
        <input id="{{$name}}" type="checkbox" name="{{$name}}" value="y" @if(!empty($required)) required @if(!empty($errorMsg))  data-error-msg="{{$errorMsg}}" @endif data-error-location="#{{$name}}ErrorMsgLocation" @endif/>
